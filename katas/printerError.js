...
function printerError(s) {
  let a = 0;  
    for(let i = s.length; i>=0;--i){   //it equals the number of times the loop occurs and the number of characters in the string
      (s[i] > "m" )? a++ : a  //sum one when the letter is above m
    }
    return `${a}/${s.length}`; 
}
